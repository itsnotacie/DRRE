...
def select_model(args, device):
    ...
    if model_id == 0:
        from models.team08_drre import main as DAT
        name = f"{model_id:02}_DAT_baseline"
        model_path = os.path.join('model_zoo', 'team00_dat', 'model.pkl')
        model_func = DAT
    elif model_id == 8:
        from models.team08_drre import main as DRRE
        name = f"{model_id:02}_DRRE_baseline"
        model_path = os.path.join('model_zoo', 'team08_drre', 'DRRE.pkl')
        model_func = DRRE
    else:
